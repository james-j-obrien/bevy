...
pub trait HierarchyQueryExt<'w, D: QueryData, F: QueryFilter> {
    ...
    /// ```
    fn iter_descendants(&'w self, entity: Entity) -> DescendantIter<'w, D, F>
    where
    ...
    /// # bevy_ecs::system::assert_is_system(system);
    /// ```
    fn iter_ancestors(&'w self, entity: Entity) -> AncestorIter<'w, D, F>
    where
        D::ReadOnly: WorldQuery<Item<'w> = &'w Parent>;
}

impl<'w, D: QueryData, F: QueryFilter> HierarchyQueryExt<'w, D, F> for Query<'w, D, F> {
    fn iter_descendants(&'w self, entity: Entity) -> DescendantIter<'w, D, F>
    where
        D::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
    {
...
///
/// Traverses the hierarchy breadth-first.
pub struct DescendantIter<'w, D: QueryData, F: QueryFilter>
where
    D::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
{
    children_query: &'w Query<'w, D, F>,
    vecdeque: VecDeque<Entity>,
}

impl<'w, D: QueryData, F: QueryFilter> DescendantIter<'w, D, F>
where
    D::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
{
    /// Returns a new [`DescendantIter`].
    pub fn new(children_query: &'w Query<'w, D, F>, entity: Entity) -> Self {
        DescendantIter {
            children_query,
...
impl<'w, D: QueryData, F: QueryFilter> Iterator for DescendantIter<'w, D, F>
where
    D::ReadOnly: WorldQuery<Item<'w> = &'w Children>,
{
    type Item = Entity;
...
pub struct AncestorIter<'w, D: QueryData, F: QueryFilter>
where
    D::ReadOnly: WorldQuery<Item<'w> = &'w Parent>,
{
    parent_query: &'w Query<'w, D, F>,
    next: Option<Entity>,
}

impl<'w, D: QueryData, F: QueryFilter> AncestorIter<'w, D, F>
where
    D::ReadOnly: WorldQuery<Item<'w> = &'w Parent>,
{
    /// Returns a new [`AncestorIter`].
    pub fn new(parent_query: &'w Query<'w, D, F>, entity: Entity) -> Self {
        AncestorIter {
            parent_query,
            next: Some(entity),
        }
...
impl<'w, D: QueryData, F: QueryFilter> Iterator for AncestorIter<'w, D, F>
where
    D::ReadOnly: WorldQuery<Item<'w> = &'w Parent>,
{
    type Item = Entity;
